...
func _process(delta):
	var size_increase = GameManager.current_insanity * max_size_increase * sin(GameManager.run_time) +GameManager.current_linear_insanity * max_size_increase
	for child in children:
		if child is CSGBox3D:
			var scale = Vector3((child.size.x + size_increase) / child.size.x, 1, (child.size.z + size_increase) / child.size.z)
			child.scale = scale
	breathing_audio_player.volume_db = (1 - GameManager.current_insanity) * -80
